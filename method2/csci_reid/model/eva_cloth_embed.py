...
class EvaAttention_sep(EvaAttention):
    def __init__(self, dim=None, unified_dist=None, **kwargs):
        super().__init__(dim=dim, **kwargs)
        self.unified_dist = unified_dist

    def calculate_attn(self, q_nth_head, q_spatial_tokens, k_nth_head, k_spatial_tokens, v_nth_head, v_spatial_tokens, attn_mask=None):
        attn_nth_head = ( torch.cat([q_nth_head, q_spatial_tokens], 2) @  torch.cat([k_nth_head, k_spatial_tokens], 2).transpose(-2, -1))
        ...
        x_nth_head = attn_nth_head @ torch.cat([v_nth_head, v_spatial_tokens], 2)
        return x_nth_head
    ...
    def forward( self, x, rope: Optional[torch.Tensor] = None, attn_mask: Optional[torch.Tensor] = None, ):
        ...
        heads = []
        if self.fused_attn:
            assert self.unified_dist is False or self.unified_dist is None, "Not yet implemented / verified"
            ...
            # k_extra_token, k_class_token, k_spatial_tokens = k[:,:,0,:], k[:,:,1,:], k[:,:,2:,:]
            # # k_extra_token.shape, k_class_token.shape, k_spatial_tokens.shape 
            # v_extra_token, v_class_token, v_spatial_tokens = v[:,:,0,:], v[:,:,1,:], v[:,:,2:,:]
            # q_extra_token, q_class_token, q_spatial_tokens = q[:,:,0,:], q[:,:,1,:], q[:,:,2:,:]
            ...
            # x_class = F.scaled_dot_product_attention(
            #     torch.cat([q_class_token.unsqueeze(2), q_spatial_tokens], 2),
            #     torch.cat([k_class_token.unsqueeze(2), k_spatial_tokens], 2),
            #     torch.cat([v_class_token.unsqueeze(2), v_spatial_tokens], 2),
            #     attn_mask=attn_mask,
            #     dropout_p=self.attn_drop.p,
            # )
            # x_class_token, x_class_spatial  = x_class[:,:,0,:], x_class[:,:,1:,:]

        elif self.unified_dist:
            assert self.num_head_tokens > 2, "Unified Distentanglement with <= 2 tokens?????"
            q = q * self.scale
            ...
            Remain_token = self.num_head_tokens - 1

            k_cls_head = k[:,:,  Remain_token  ,:] 
            v_cls_head = v[:,:,  Remain_token  ,:] 
            q_cls_head = q[:,:,  Remain_token  ,:] 
            # print(k_cls_head.shape, v_cls_head.shape, q_cls_head.shape)

            k_remain_head = k[:,:,  :Remain_token  ,:] 
            v_remain_head = v[:,:,  :Remain_token  ,:] 
            q_remain_head = q[:,:,  :Remain_token  ,:] 
            # print(k_remain_head.shape, v_remain_head.shape, q_remain_head.shape)
            
            
            x_cls_head = self.calculate_attn( q_nth_head=q_cls_head.unsqueeze(2), q_spatial_tokens=q_spatial_tokens, 
                k_nth_head=k_cls_head.unsqueeze(2), k_spatial_tokens=k_spatial_tokens, 
                v_nth_head=v_cls_head.unsqueeze(2), v_spatial_tokens=v_spatial_tokens, 
                attn_mask=attn_mask, )
            x_remain_head = self.calculate_attn(q_nth_head=q_remain_head, q_spatial_tokens=q_spatial_tokens, 
                k_nth_head = k_remain_head, k_spatial_tokens=k_spatial_tokens, 
                v_nth_head=v_remain_head, v_spatial_tokens=v_spatial_tokens, 
                attn_mask=attn_mask, )
            
            x_cls_head_token, x_cls_head_spatial  = x_cls_head[:,:,  0 ,:], x_cls_head[:,:,1:,:]
            # x_cls_head_token.shape, x_cls_head_spatial.shape
            x_remain_head_token, x_remain_head_spatial  = x_remain_head[:,:,  :Remain_token ,:], x_remain_head[:,:,Remain_token:,:]
            # x_remain_head_token.shape, x_remain_head_spatial.shape

            x_spatial = x_cls_head_spatial + x_remain_head_spatial
            heads = [x_remain_head_token, x_cls_head_token.unsqueeze(2)]
        else:
            q = q * self.scale
        ...
        if self.unified_dist:
            x_spatial = x_spatial / (self.num_head_tokens - 1)
        else:
        ...
        # attn_extra = ( torch.cat([q_extra_token.unsqueeze(2), q_spatial_tokens], 2) @  torch.cat([k_extra_token.unsqueeze(2), k_spatial_tokens], 2).transpose(-2, -1))
        # attn_extra = attn_extra.softmax(dim=-1)

        # attn_class = ( torch.cat([q_class_token.unsqueeze(2), q_spatial_tokens], 2) @  torch.cat([k_class_token.unsqueeze(2), k_spatial_tokens], 2).transpose(-2, -1))
        # attn_class = attn_class.softmax(dim=-1)
        
        
        # attn_extra = self.attn_drop(attn_extra)
        # attn_class = self.attn_drop(attn_class)
        
        # x_extra = attn_extra @ torch.cat([v_extra_token.unsqueeze(2), v_spatial_tokens], 2)
        # x_class = attn_class @ torch.cat([v_class_token.unsqueeze(2), v_spatial_tokens], 2)

        # x_extra_token, x_extra_spatial  = x_extra[:,:,0,:], x_extra[:,:,1:,:]
        # x_class_token, x_class_spatial  = x_class[:,:,0,:], x_class[:,:,1:,:]

        # x_spatial = (x_extra_spatial + x_class_spatial) / 2
        # x = torch.cat([x_extra_token.unsqueeze(2),x_class_token.unsqueeze(2), x_spatial],2)
...
class EvaAttention_sep_masked(EvaAttention):
    def __init__(self, dim=None, **kwargs):
        super().__init__(dim=dim, **kwargs)
        HEIGHT=256
        self.attn_mask = torch.zeros(HEIGHT + self.num_head_tokens,HEIGHT + self.num_head_tokens, requires_grad=False)
        for nth_head_token in range(self.num_head_tokens):
            self.attn_mask[nth_head_token][:nth_head_token] += float("-inf")
            self.attn_mask[nth_head_token][nth_head_token+1:self.num_head_tokens]  += float("-inf")
...
class EvaBlock(nn.Module):
    ...
    def __init__(
        ...
        if num_head_tokens == 1 or not (extra_token_separation):
            if num_head_tokens != 1:
                print(" ***  Same Attention for Extra Token (Entanglement) .....  *** ")
            ...
            if masked_sep_attn:
                print(" ***  Using Unified Masked Sep Attention  .....  *** ")
                self.attn = EvaAttention_sep_masked(
                    dim,
                    num_heads=num_heads,
                    qkv_bias=qkv_bias,
                    qkv_fused=qkv_fused,
                ...
                print(" ***  Separate Attention .....  *** ")
                self.attn = EvaAttention_sep(
                    dim,
                    num_heads=num_heads,
                    qkv_bias=qkv_bias,
                    qkv_fused=qkv_fused,
...
class Eva(nn.Module):
    ...
    def __init__(
        ...
        if num_head_tokens != 1:
            self.num_prefix_tokens = num_head_tokens
        ...
        self.blocks = nn.ModuleList([
            block_fn(
                ...
                num_head_tokens=num_head_tokens, 
                extra_token_separation=extra_token_separation, 
                masked_sep_attn=masked_sep_attn,
            )
        ...
        if spatial_avg:
            ...
            self.spatial_pool = pooling.MaxAvgPooling()
            use_fc_norm = True 
        ...
        self.ignore_list = []
        if not (config.MODEL.CLOTH_EMBED):
            self.ignore_list.append("cloth_embed")
    ...
    def forward_spatial_head(self, x, pre_logits: bool = False):
    ...
    def forward(self, x,cloth_id=None):
        ...
        if self.spatial_avg:
            feat = self.forward_spatial_head(x, pre_logits=True)
        else:
    ...
    def load_param(self, trained_path, load_head=True):
        ...
        for i in param_dict:
            param_key = i.replace('module.', '')
            if param_key in self.ignore_list:continue 
            self.state_dict()[ param_key ].copy_(param_dict[i])
...
class Eva_TA(Eva):
    ...
    def forward_features(self, x,cloth_id):
        ...
        # apply abs position embedding
        if self.pos_embed is not None:
            # cloth_embed = self.cloth_embed[cloth_id]
            # cloth_embed = repeat(cloth_embed, "B ... -> B N ... ", N=T)
            # cloth_embed = rearrange(cloth_embed, "B T C D -> (B T) C D")
            # x = x + self.pos_embed + self.cloth_xishu * cloth_embed
            x = x + self.pos_embed 
        x = self.pos_drop(x)
    ...
    def forward(self, x,cloth_id):
        ...
        feat = self.forward_head(x, B, T, pre_logits=True)
        if not self.training:
            return feat
        else:
            cls_score = self.head(feat)
...
@register_model
def eva02_large_patch14_224_cloth(pretrained=False, **kwargs) -> Eva:
    """ A EVA-CLIP specific variant that adds additional attn scale layernorm to eva02_large """
    model_args['global_pool'] = kwargs.pop('global_pool', 'token')
...
@register_model
def eva02_vid_TA(pretrained=False, **kwargs) -> Eva:
    """ A EVA-CLIP specific variant that adds additional attn scale layernorm to eva02_large """
    model_args['global_pool'] = kwargs.pop('global_pool', 'token')
    if kwargs.get('features_only', None):
        raise RuntimeError('features_only not implemented for Eva models.')
    ...
    model = build_model_with_cfg(Eva_TA, variant, pretrained=pretrained, pretrained_filter_fn=checkpoint_filter_fn, **kwargs)
    return model

    
@register_model
def eva02_small_cloth(pretrained=False, **kwargs) -> Eva:
    model_args['global_pool'] = kwargs.pop('global_pool', 'token')
    model_args['embed_dim'] = 384
    model_args['depth'] = 12

    # {'img_size': 224, 'patch_size': 14, 'embed_dim': 1024, 'depth': 24, 'num_heads': 16, 'mlp_ratio': 2.6666666666666665, 'qkv_fused': False, 'swiglu_mlp': True, 'scale_mlp': True, 'scale_attn_inner': True, 'use_rot_pos_emb': True, 'ref_feat_shape': (16, 16), 'cloth': 300, 'global_pool': 'token'}
...
@register_model
def eva02_base_cloth(pretrained=False, **kwargs) -> Eva:
    model_args['global_pool'] = kwargs.pop('global_pool', 'token')
