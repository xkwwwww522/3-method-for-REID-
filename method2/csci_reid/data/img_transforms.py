...
class RandomTilT(object):
    def __init__(self, angle):
        self.angle = range(-angle, angle)
        self.p= 0.5
        
        mean = (0.4914, 0.4822, 0.4465)
        self.filler = (int(mean[0]*256), int(mean[1]*256), int(mean[2]*256))

    def __call__(self, img):
        if random.random() < 0.5:
            angle = random.choice(self.angle)
            im_rotate = img.rotate(angle, PIL.Image.NEAREST, fillcolor = self.filler, expand=True)
            return im_rotate
        return img
