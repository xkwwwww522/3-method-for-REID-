...
class RGBuvHistBlock(nn.Module):
  ...
  def forward(self, x):
    ...
    hists = torch.zeros((x_sampled.shape[0], 1 + int(not self.green_only) * 2, self.h, self.h)).to(device=self.device)
    l = 0
...
class RGBuvHistBlock_Original(nn.Module):
  def __init__(self, h=64, insz=150, resizing='interpolation',
               method='inverse-quadratic', sigma=0.02, intensity_scale=True,
               hist_boundary=None, green_only=False, device='cuda'):
    ...
    Methods:
      forward: accepts input image and returns its histogram feature. Note that
        ...
        and can be easily integrated with the loss function. As mentioned in the
         ...
         training.
    """
    super(RGBuvHistBlock_Original, self).__init__()
    self.h = h
    self.insz = insz
    self.device = device
    self.resizing = resizing
...
if __name__ == "__main__":
    ...
    histblock = RGBuvHistBlock(insz=224, h=64, resizing='sampling', method='inverse-quadratic', sigma=0.02, device='cpu')
    histblock(x)
