...
def split_path_mevid(row):
    sil_path = row.Path.split("/")[-1]    
    sil_path = sil_path.replace("_sil", "").replace(".png", ".jpg")
    return sil_path

class MEVID(object):
    """ MEVID
    """
    def __init__(self, root='../../mevid/', sampling_step=64, seq_len=8, stride=4, Load_As_Image=None, test_mode=None, 
        load_test = True, **kwargs):
        # print(sampling_step,  seq_len, stride) 64 8 4
        self.root = root
        ...
        if load_test:
            ...
            num_gallery_imgs, num_query_imgs, num_gallery_pids, clothes2label = [], [], 0, [] 
            num_gallery_tracklets, num_query_tracklets = 0,0 
            query, gallery, recombined_query, recombined_gallery = [], [], [], []
            query_vid2clip_index = None
            gallery_vid2clip_index = None 
            num_query_pids, num_query_clothes, num_gallery_clothes = 0, 0, 0 
    ...
    def _densesampling_for_trainingset(self, dataset, sampling_step=64):
        ...
        '''
        new_dataset = []
        for (img_paths, pid, camid, clothes_id, attribute) in dataset:
            if sampling_step != 0:
                num_sampling = len(img_paths)//sampling_step
                if num_sampling == 0:
                    new_dataset.append((img_paths, pid, camid, clothes_id, attribute))
                else:
                    for idx in range(num_sampling):
                        if idx == num_sampling - 1:
                            new_dataset.append((img_paths[idx*sampling_step:], pid, camid, clothes_id, attribute))
                        else:
                            new_dataset.append((img_paths[idx*sampling_step : (idx+1)*sampling_step], pid, camid, clothes_id, attribute))
            else:
                new_dataset.append((img_paths, pid, camid, clothes_id, attribute))
    ...
    def _recombination_for_testset(self, dataset, seq_len=16, stride=4):
        ...
        for idx, (img_paths, pid, camid, clothes_id, attribute) in enumerate(dataset):
            # start index
            ...
            for i in range(len(img_paths)//(seq_len*stride)):
                for j in range(stride):
                    ...
                    new_dataset.append((clip_paths, pid, camid, clothes_id, attribute))
...
class MEVID_CLEAN(MEVID):
    def __init__(self, **kwargs):
        self.train_csv="Scripts/Helper/MeVID/"        
    ...
    def _process_mevid_data(self, track_names, track_info, home_dir, relabel=False, clothes2label=None):
        ...
        for sta_idx, end_idx, pid, clothes_label, camid in tracklet_path_list:
            ...
            if len(img_names) == 0:
                continue
            if curr_pid is None or curr_pid != pid:    
                df = os.path.join(self.train_csv, "{:>04}".format(str(pid)) + ".csv")
                if os.path.exists(df) and "train" in home_dir: 
                    df =  pd.read_csv(df)
                    df["files_name"] = df.apply(split_path_mevid, axis=1)
                    curr_read = set(df["files_name"])
                    curr_pid = pid
                else:
                    curr_read = img_names
                    
            img_names = [e for e in img_names if e in curr_read]
            if img_names == []:
                continue 
            img_paths = [osp.join(self.root, home_dir, img_name[:4], img_name) for img_name in img_names]
