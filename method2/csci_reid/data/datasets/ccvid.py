...
class CCVID(object):
    ...
    """
    def __init__(self, root='/data/datasets/', sampling_step=64, seq_len=8, stride=4, test_mode=None, **kwargs):
        ...
        logger.info("   train.txt    |     0 |  13799  |   49577  |   55237   |      0 ") 
        logger.info("   query.txt    |     0 |  26702  |   41930  |   48167   |      0 ") 
        logger.info("   gallery.txt  |     0 |  11776  |   32958  |   67687   |      0 ") 
        logger.info("    ---------------------------------------------------------------")
    ...
    def _densesampling_for_trainingset(self, dataset, sampling_step=64):
        ...
        '''
        new_dataset = []
        for (img_paths, pid, camid, clothes_id, attr) in dataset:
            if sampling_step != 0:
                num_sampling = len(img_paths)//sampling_step
                if num_sampling == 0:
                    new_dataset.append((img_paths, pid, camid, clothes_id, attr))
                else:
                    for idx in range(num_sampling):
                        if idx == num_sampling - 1:
                            new_dataset.append((img_paths[idx*sampling_step:], pid, camid, clothes_id, attr))
                        else:
                            new_dataset.append((img_paths[idx*sampling_step : (idx+1)*sampling_step], pid, camid, clothes_id,  attr))
            else:
                new_dataset.append((img_paths, pid, camid, clothes_id, attr))
    ...
    def _recombination_for_testset(self, dataset, seq_len=16, stride=4):
        ...
        for idx, (img_paths, pid, camid, clothes_id, attr) in enumerate(dataset):
            # start index
            ...
            for i in range(len(img_paths)//(seq_len*stride)):
                for j in range(stride):
                    ...
                    new_dataset.append((clip_paths, pid, camid, clothes_id, attr))
...
if __name__ =='__main__':
    # cd ~/MADE_ReID/
    # python data/datasets/ccvid.py 
    dataset=CCVID_print_middle_frames(root="/data/priyank/synthetic/CCVID/", aux_info=None, meta_dir=None, meta_dims=105, test_mode=False)
