"""Exhaustive Classifier(751) + ReRank sweep."""
import sys
sys.path.insert(0, '.')
...
model.load_param(cfg.TEST.WEIGHT); model.to(device)
model.train()
clf = []; ap = []; ac = []
for img, pid, camid, camids, view, impath in vl:
    with torch.no_grad():
        img = img.to(device)
        sl, fl, if_ = model(img, label=torch.zeros(img.size(0), dtype=torch.long, device=device))
        clf.append(sl[0].cpu())
...
print('Classifier(751) baseline: mAP=%.1f%% R1=%.1f%%' % (mc*100, cc[0]*100))

best = (mc, cc[0], 0, 0.0)
for k1 in [3,4,5,6,7,8,9,10,12,15,20,25,30,40,50]:
    k2 = max(2, k1//3)
    for lam in [0.03, 0.05, 0.08, 0.10, 0.12, 0.15, 0.18, 0.20, 0.25, 0.30, 0.40, 0.50]:
        try:
            dr = re_ranking(clf[:nq], clf[nq:], k1=k1, k2=k2, lambda_value=lam)
            cm, m = eval_func(dr, qp, gp, qc, gc)
            if m > best[0]:
                best = (m, cm[0], k1, lam)
                print('  k1=%2d lam=%.2f: mAP=%.1f%% R1=%.1f%%' % (k1, lam, m*100, cm[0]*100))
        except: pass
