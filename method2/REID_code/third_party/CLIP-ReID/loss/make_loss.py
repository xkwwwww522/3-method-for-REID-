...
def make_loss(cfg, num_classes):    # modified by gu
    ...
    if sampler == 'softmax':
        # 用 *args 和 **kwargs 吸收掉所有多余的参数（如 target_cam, logits），防止报 5 个参数的错
        def loss_func(score, feat, target, *args, **kwargs):
            # 兼容 Label Smoothing
            if cfg.MODEL.IF_LABELSMOOTH == 'on':
                if isinstance(score, list):
                    return sum([xent(s, target) for s in score])
                else:
                    return xent(score, target)
            # 不用 Label Smoothing 的情况
            else:
                if isinstance(score, list):
    ...
    elif 'softmax_triplet' in sampler or 'clothes_aware' in sampler:
