...
# LoRA rank for fine-tuning (set to 0 to disable LoRA injection)
_C.MODEL.LORA_R = 16
# Number of top ViT layers to unfreeze for full fine-tuning (0 = LoRA only)
_C.MODEL.UNFREEZE_VIT_LAYERS = 0
...
# Min area ratio for random erasing
_C.INPUT.RE_MIN_AREA = 0.02
# Max area ratio for random erasing
_C.INPUT.RE_MAX_AREA = 0.33
...
_C.INPUT.PADDING = 10
# Use RandomResizedCrop instead of Resize+Pad+RandomCrop for multi-scale training
_C.INPUT.USE_RRC = False
# Scale range for RandomResizedCrop (min, max fraction of original area)
_C.INPUT.RRC_SCALE_MIN = 0.3
_C.INPUT.RRC_SCALE_MAX = 1.0
# Aspect ratio range for RandomResizedCrop
_C.INPUT.RRC_RATIO_MIN = 0.3
_C.INPUT.RRC_RATIO_MAX = 1.5
# RandomGrayscale probability (0.0 = disabled)
_C.INPUT.RANDOM_GRAYSCALE = 0.0
# Head-area targeted erasing probability (top 1/3 of image)
_C.INPUT.HEAD_ERASE_PROB = 0.0
# Horizontal stripe erasing probability
_C.INPUT.STRIPE_ERASE_PROB = 0.0
# Stripe height ratio for stripe erasing
_C.INPUT.STRIPE_ERASE_RATIO = 0.5
# Local brightness perturbation probability
_C.INPUT.BRIGHTNESS_PERTURB_PROB = 0.0
# Brightness perturbation factor (lower = darker)
_C.INPUT.BRIGHTNESS_PERTURB_FACTOR = 0.4
...
# Number of train frames per tracklet
_C.DATALOADER.NUM_TRAIN_FRAMES = 20
